...
var volt_needle_rotate_gh,
  ammeter_needle_rotate_gh,
  volt_needle_rotate_mh,
  ammeter_needle_rotate_mh;
...
var anim_frame,
  anim_width,
  anim_object_top,
  anim_object_bottom; /** Variables for animation */
...
var gh_count,
  mh_count,
  conductivity_blackbody,
  t4_chamber_temp,
  t1_to_t3_blackbody_temp,
  t5_to_t7_testplate_temp;

var vneedle_distance_interval_gh,
  aneedle_distance_interval_gh,
  vneedle_distance_interval_mh,
  aneedle_distance_interval_mh;
...
var mask_rect_anim =
  new createjs.Shape(); /** Add rectangle shape for water animation masking */
  ...
  return {
    ...
    link: function (scope, element, attrs, dialogs) {
      ...
      function handleComplete() {
        ...
        anim_object_top =
          conduction_stage.getChildByName(
            "waterAnimationTop",
          ); /** Top animation frames take as an object */
        anim_object_bottom = conduction_stage.getChildByName(
          "waterAnimationBottom",
        ); /** Bottom animation frames take as an object */

        conduction_stage.getChildByName("switch_off").on("click", function () {
          /** Motor switch on functionality */
          scope.powerOnBtn(); /** Start the experiment with respect to the power on button */
          scope.$apply();
        });
        conduction_stage.getChildByName("switch_on").on("click", function () {
          /** Motor switch off functionality */
          resetExperiment(); /** Reset the experiment */
          scope.$apply();
        });
        conduction_stage.getChildByName("MH_switch").on("click", function () {
          /** Toggle switch turn to GH */
          ...
          if (gh_voltage_int == 0) {
            /** Rotation of ammeter and voltmeter needle on switch to GH */
            volt_needle_rotate_gh = gh_voltage_int / 6.67;
            ammeter_needle_rotate_gh = gh_voltage_int / 10;
          }
          rotateNeedlesGH(
            volt_needle_rotate_gh,
            ammeter_needle_rotate_gh,
          ); /** Rotation of needles */
        });
        conduction_stage.getChildByName("GH_switch").on("click", function () {
          /** Toggle switch turn to GH */
          ...
          if (mh_voltage_int == 0) {
            /** Rotation of ammeter and voltmeter needle on switch to MH */
        ...
        conduction_stage.addChild(
          mask_rect_anim,
        ); /** Adding that rectangle to the stage */
      }
      ...
      function translationLabels() {
        ...
        scope.material_array = [
          { material: _("Cardboard"), type: 0 },
          { material: _("Glass"), type: 1 },
          { material: _("Mica"), type: 2 },
          { material: _("Asbestos-cement board"), type: 3 },
          { material: _("Ebonite solid"), type: 4 },
        ];
        scope.$apply();
      }
    },
...
function loadImages(image, name, xPos, yPos, cursor, rot) {
  ...
  if (name == "waterAnimationTop" || name == "waterAnimationBottom") {
    /** Masking of water animation */
...
function showCrossSectionFN(scope) {
  if (scope.crossSectionValue == true) {
    conduction_stage.getChildByName("cross_section").visible =
      true; /** Display cross sectional view */
    conduction_stage.getChildByName("waterAnimationTop").visible =
      true; /** Display the top water animation */
    conduction_stage.getChildByName("waterAnimationBottom").visible =
      true; /** Display the bottom water animation */
    ...
    conduction_stage.getChildByName("cross_section").visible =
      false; /** Cross sectional view visibility false */
    conduction_stage.getChildByName("waterAnimationTop").visible =
      false; /** Top water animation visibility false */
    conduction_stage.getChildByName("waterAnimationBottom").visible =
      false; /** Bottom water animation visibility false */
...
function startAnimation() {
  ...
  if (anim_frame <= 8) {
    anim_object_top.x =
      anim_object_top.x -
      anim_width; /** Changing of top animation object x position */
    anim_object_bottom.x =
      anim_object_bottom.x -
      anim_width; /** Changing of bottom animation object x position */
  } else {
...
function rotateNeedlesGH(val1, val2) {
  if (toggle_flag) {
    ...
    conduction_stage.getChildByName("ammeterReading").text =
      gh_current_int.toFixed(2);
  }
}
...
function indicatorReadingFn() {
  conduction_stage.getChildByName("indicatorReading").text =
...
function thermalConductivityCalc() {
  ...
  area_float = (
    (Math.PI * diameter_val_float * diameter_val_float) /
    4
  ).toFixed(5);

  /** Ta=(q*dx*10^-2)/(k*A)+T6 */
  temperature_a = (
    ((gh_power_int / 2) * thickness_float) / (conductivity_float * area_float) +
    temperature_val
  ).toFixed(3);
  ...
  t1_temperature = parseFloat(
    temperature_a - rand_a,
  ); /** T1 temperature value */
  t2_temperature = parseFloat(
    temperature_a + rand_a,
  ); /** T2 temperature value */

  t3_and_t7_temperature = parseFloat(
    temperature_b - rand_b,
  ); /** T3 and T7 temperature value */
  t4_and_t8_temperature = parseFloat(
    temperature_b + rand_b,
  ); /** T4 and T8 temperature value */
  ...
  } else if (indicator_text_count == 3 || indicator_text_count == 7) {
